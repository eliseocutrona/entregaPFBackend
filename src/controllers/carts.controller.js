...
const purchaseCart = async (req, res) => {
    ...
    try {
        ...
        for (const item of cartProducts) {
            const product = await productsService.getById(item.productId); // Corrige el método para obtener productos
            if (!product) {
                outOfStock.push({ id: item.productId, quantity: item.quantity, available: 0 });
                continue;
            }
        ...
        // Si hay productos fuera de stock, devolver error
        ...
        // Si todos los productos están en stock
        if (inStock.length === 0) {
            return res.status(406).json({
                status: "error",
        ...
        // Calcular el total y generar el ticket
        ...
        // Responder con el ticket generado
        res.status(201).json({
            status: "success",
